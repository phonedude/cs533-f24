...
This literary reference is in regard to Coleridge's "Rime of the Ancient Mariner" which is a book about a group of sailors who were stranded in the ocean and become dehydrated. The book has a quote that is "Water, water, everywhere, Nor any drop to drink." which means that the stranded sailors can't drink the water surrounding them because it's ocean's saltwater will kill them if they drink it. For our Web Security course the reason this relates to the subject of XSS is because there are so many different parsers that developers have to employ which can be hard to know how they will interact because of how complex their interrelationships are with user input. When input isn't parsed properly we have learned that increased vulnerabilities to things like XSS are more probable.
